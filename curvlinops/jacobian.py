...
def make_batch_transposed_jacobian_matrix_product(
    ...
    r"""Set up function to multiply the mini-batch transposed Jacobian onto a matrix.

    Args:
        model_func: The neural network :math:`f_{\mathbf{\theta}}`.
        params: A tuple of parameters w.r.t. which the transposed Jacobian is computed.
            All parameters must be part of ``model_func.parameters()``.
    ...
    """
    # detect the parameters w.r.t. which the transposed Jacobian is computed
    free_param_names = []
    for p in params:
        (name,) = [n for n, pp in model_func.named_parameters() if pp is p]
        free_param_names.append(name)
...
class TransposedJacobianLinearOperator(CurvatureLinearOperator):
    ...
    def _matmat(self, M: List[Tensor]) -> List[Tensor]:
        ...
        """
        if not hasattr(self, "_jtmp"):
            self._jtmp = make_batch_transposed_jacobian_matrix_product(
                self._model_func, tuple(self._params)
            )
        ...
        for X, _ in self._loop_over_data(desc="_matmat"):
            ...
            JTM_batch = self._jtmp(X, M_batch)
