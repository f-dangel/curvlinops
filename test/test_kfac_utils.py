...
@mark.parametrize(
    "output_shape", [(3,), (3, 4), (3, 2, 2)], ids=lambda s: f"{len(s)}d_sequence"
)
@mark.parametrize(
    "loss_func_cls",
...
def test_loss_hessian_matrix_sqrt(
    ...
    Raises:
        NotImplementedError: If an unsupported loss function is provided.
    """
    manual_seed(0)
...
def test_grad_output_sampler_convergence(
    ...
    flat_dim = output_one_datum.numel()
    grad_samples_flat = grad_samples.reshape(mc_samples, flat_dim)
    empirical_cov = grad_samples_flat.T @ grad_samples_flat / mc_samples
    ...
    assert allclose_report(
        empirical_cov / scale, true_hess / scale, rtol=1e-2, atol=5e-3
    )
