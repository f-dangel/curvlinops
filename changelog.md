...
- Support plain callable `(params_dict, X) -> prediction` as `model_func` in
  `GGNLinearOperator`, `HessianLinearOperator`, `EFLinearOperator`,
  `KFACLinearOperator`, and `EKFACLinearOperator` (KFAC/EKFAC require
  `backend="make_fx"`).
  Pass `params` as `dict[str, Tensor]` instead of `list[Parameter]`
  ([PR](https://github.com/f-dangel/curvlinops/pull/271),
  [PR](https://github.com/f-dangel/curvlinops/pull/274))
